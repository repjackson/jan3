Meteor.methods
    ...
    create_ticket_event: (ticket_doc_id, event_type, action)->
        ...
        office_doc = Meteor.call 'find_office_from_customer_jpid', ticket.customer_jpid
        franchisee_doc = Meteor.call 'find_franchisee_from_customer_jpid', ticket.customer_jpid
        Docs.insert
            type:'event'
            parent_id: ticket_doc_id
            event_type: event_type
            action:action
            office_jpid: office_doc.office_jpid
            franchisee_jpid: franchisee_doc.franchisee_jpid
    ...
    find_office_from_customer_jpid: (customer_jpid)->
        ...
        if customer_doc
            found_office = Docs.findOne
                office_name: customer_doc.office_name
                type:'office'
            return found_office
        else
    ...
    find_franchisee_from_customer_jpid: (customer_jpid)->
        customer_doc = Docs.findOne
            "ev.ID": customer_jpid
            type:'customer'
        if customer_doc and customer_doc.franchisee_name
            found_franchisee = Docs.findOne
                franchisee_name: customer_doc.franchisee_name
                type: 'franchisee'
    ...
    refresh_customer_jpids: (username)->
        ...
        found_office = Docs.findOne
            office_name: found_franchisee.office_name
            type: 'office'
